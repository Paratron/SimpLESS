...
define(['less', '3p/cleancss'], function (less, cleancss) {
    ...
    function prefix(model,callBack){
    ...
    function compile(model){
        compile_to_string(model,function(model,csscode){
            save_css_file(model,csscode);
        });
    }

    function compile_to_string(model,callBack){
        ...
        try {
            parser.parse(lesscode, function (err, tree) {
                ...
                if (model_dta.settings.prefix) {
                    prefix(model, function(model,csscode){
                        callBack(model, csscode);
                    });
                } else {
                    //Do not prefix.
                    callBack(model, csscode);
    ...
    var obj = {
        compile:compile,
        compile_to_string:compile_to_string
    }
